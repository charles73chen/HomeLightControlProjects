...
void send_light_state() {
  long now = millis();
  char json[50] = "{\"id\":\"home.liveroom.tv\",\"value\":\"1\"}" ;
  if (now - lastMsg > 5000) {
    lastMsg = now;

    //sprintf(msg, "%d", digitalRead(2));
    json[26] = (char)digitalRead(2);
    client.publish("home.light.state.update", json );
...
void ScanNetwork() {
  ...
  st += "</select><select name'passtype'><option></option>";
